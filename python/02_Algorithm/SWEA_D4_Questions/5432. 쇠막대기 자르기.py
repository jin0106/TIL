...
for t in range(1, T+1):
    ...
    i = 0
    while i < len(metal):
        if metal[i] == "(" and metal[i+1] == ')':  # 레이저일때
            ans += cnt  # 쇠막대기 갯수 만큼 더해주고 레이저 뒤부터 다시 검사
            i += 2
        elif metal[i] == "(":   # 레이저가 아닌 (일때 쇠막대기를 하나씩 추가
            cnt += 1
            i += 1
        elif metal[i] == ")":   # 레이저가 아닌 ) 일때 쇠막대기가 끝났으므로 cnt -=1 ans +=1
            cnt -= 1
            ans += 1
            i += 1
    print(f'#{t} {ans}')
