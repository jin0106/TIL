...
for t in range(1, T+1):
    ...
    all_student = list(range(1,N+1))    # 모든 학생 리스트
    submit = list(map(int, input().split()))    # 제출한 학생 리스트
    not_submit = [] # 제출 안한 학생 리스트 생성
    for i in range(N):
        if all_student[i] not in submit:    # 모든 학생 중에 제출한 학생 리스트에 없으면 제출 안한 리스트에 추가
            not_submit.append(all_student[i])
    print(f"#{t} {' '.join(map(str, not_submit))}")
